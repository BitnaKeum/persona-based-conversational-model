...
class Seq2SeqDataCollator:
    ...
    def preprocess_batch(self, batch, context_size, mem_size):
        speaker_pattern = re.compile('<[a-z]{4,5}>')  # <user>, <agent>
        persona_pattern = re.compile('(<agent_persona>|<user_persona>)')

        # source로부터 persona 분리하기
        new_batch = []
        for dict_data in batch:
            source = dict_data["src_texts"].replace('\n', '')

            # source에서 persona를 분리
            speaker_result = speaker_pattern.finditer(source)
            context_start_token = ''
            for r in speaker_result:
                ...
                if '<REF>' in persona:
                    ...
                    for persona_idx, persona in enumerate(personas):
                        if '<REF>' in persona:
                            ...
                            break
                    personas = random.sample(personas, mem_size-1)
                    personas.append(ref_persona)
                else:
    ...
    def preprocess_persona(self, persona_ids_list, persona_attn_mask_list, memory_labels, mem_size, cxt_size):
        # batch 내 max_seq_len 구하기
        max_seq_len = 0
        for persona_ids in persona_ids_list:
            seq_len = persona_ids.shape[1]
            if seq_len > max_seq_len:
                max_seq_len = seq_len

        new_memory_labels = []
        for batch_num, (persona_ids, persona_attn_mask) in enumerate(zip(persona_ids_list, persona_attn_mask_list)):
            # padding as max_seq_len
            seq_len = persona_ids.shape[1]
            pad_cnt = max_seq_len - seq_len
            if pad_cnt > 0:
                new_persona_ids = F.pad(persona_ids, (0, pad_cnt), 'constant', self.pad_token_id)
                new_persona_attn_mask = F.pad(persona_attn_mask, (0, pad_cnt), 'constant', 0)
            else:
                new_persona_ids = persona_ids
                new_persona_attn_mask = persona_attn_mask
            ...
            if add_cnt > 0:
                ...
                add_mem_attn_mask[:, 2] = 1  # </s>
                new_persona_ids = torch.cat([new_persona_ids, add_mem_ids], dim=0)
                new_persona_attn_mask = torch.cat([new_persona_attn_mask, add_mem_attn_mask], dim=0)
            ...
            # batch 합치기
            new_persona_ids = torch.unsqueeze(new_persona_ids, dim=0)  # (1, mem_size+1, max_seq_len)
            new_persona_attn_mask = torch.unsqueeze(new_persona_attn_mask, dim=0)  # (1, mem_size+1, max_seq_len)
            if batch_num == 0:
                persona_ids_encoding = new_persona_ids
                persona_attn_mask_encoding = new_persona_attn_mask
            else:
                persona_ids_encoding = torch.cat([persona_ids_encoding, new_persona_ids], dim=0)
                persona_attn_mask_encoding = torch.cat([persona_attn_mask_encoding, new_persona_attn_mask], dim=0)

        memory_labels = []
        for memory_label in new_memory_labels:
            memory_labels.append([memory_label] * cxt_size)
        memory_labels = torch.tensor(memory_labels) # (batch, cxt_size)
    ...
    def _encode(self, batch) -> Dict[str, torch.Tensor]:
        ...
        batch_encoding = self.tokenizer(
            ['<s>' + x["src_texts"].split("<eos>")[0] + '</s>' for x in batch],
            max_length=self.data_args.max_source_length,
            padding="max_length" if self.tpu_num_cores is not None else "longest",
            return_tensors="pt",
            **self.dataset_kwargs,
        )
        # assert batch_encoding['input_ids'].size(1) < self.data_args.max_source_length, \
        #     f"There is sequence that exceeds max_source_length!\n{batch}"
        ...
        # persona encoding
        persona_ids_list, persona_attn_mask_list = [], []
        for idx, x in enumerate(batch):
            try:
                persona_inputs = self.tokenizer(
                    ['<s>' + mem.split("<eos>")[0] + '</s>' for mem in x['personas']],
                    padding="max_length" if self.tpu_num_cores is not None else "longest",
                    return_tensors="pt",
                    **self.dataset_kwargs,
                )
        ...
        # preprocess persona
        memory_labels = [one_dict["memory_label"] for one_dict in batch]
        persona_ids_encoding, persona_attn_mask_encoding, memory_labels = self.preprocess_persona(
            persona_ids_list, persona_attn_mask_list, memory_labels, mem_size=MEM_SIZE, cxt_size=CONTEXT_SIZE,
        )

...
class Trainer:
        ...
        **kwargs,
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                # inputs: {'input_ids': (batch, src_len), 'attention_mask': (batch, src_len), 'labels': (batch, tgt_len)}
    ...
    def _maybe_log_save_evaluate(self, tr_loss, model, trial, epoch, ignore_keys_for_eval):
        ...
        metrics = None
        # if self.control.should_evaluate: # evaluation 스킵
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        """
        model.train()
        inputs = self._prepare_inputs(inputs) # inputs: {'input_ids', 'attention_mask', 'labels', 'persona_ids', 'persona_attention_mask', 'memory_labels' }
    ...
    def evaluation_loop(
        ...
        if all_losses is not None:
            ...
            metrics[f"{metric_key_prefix}_perplexity"] = round(np.exp(all_losses.mean()).item(), 5)
